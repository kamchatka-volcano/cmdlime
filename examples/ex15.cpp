...
struct Cfg : public cmdlime::Config {
    ...
    CMDLIME_SUBCOMMAND(record, RecordCfg) << "record search result" << [](const auto& record) {
        if (record.file.empty() && record.db.empty())
...
int mainApp(const Cfg& cfg)
{
    ...
    if (cfg.record.has_value())
        std::cout << "Record settings: " << "file:" << cfg.record->file << " db:" << cfg.record->db
                  << " detailed:" << cfg.record->detailed << std::endl;
